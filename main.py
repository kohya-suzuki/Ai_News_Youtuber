...
ROBOT_BASE_PATH = os.path.join(BASE_DIR, "images/base_ai_robot.jpg")  # 背景込みのロボット
ROBOT_BLINK_PATH = os.path.join(BASE_DIR, "images/closed_eye.jpg")   # 瞬き用の目閉じ画像
LIGHTBULB_PATH = os.path.join(BASE_DIR, "images/lightbulb_icon.png")  # 💡アイコン
FONT_PATH = os.path.join(BASE_DIR, "fonts/HiraginoMaruGothic.ttc")  # ローカルにコピーしたヒラギノ丸ゴ

# ==========================================
# 2. 📊 テスト用擬似データ抽出（GAS/Spreadsheet想定）
# ==========================================
mock_data = {
    ...
    "news_list": [
        {
            ...
            "image_path": os.path.join(BASE_DIR, "images/news1_pexels.jpg")
        },
        {
            ...
            "image_path": os.path.join(BASE_DIR, "images/news2_pexels.jpg")
        },
        {
            ...
            "image_path": os.path.join(BASE_DIR, "images/news3_pexels.jpg")
        }
    ]
}
...
def generate_audio_timeline(data):
    ...
    for i, news in enumerate(data["news_list"]):
        ...
        news_audios.append(n_audio.with_start(start_t))
        timeline.append({"index": i, "start": start_t, "end": end_t})
        current_time = end_t
        
    guchi_start = current_time
    guchi_audio = guchi_audio.with_start(guchi_start)
...
def render_video(final_audio, timeline, total_duration, guchi_start, data):
    ...
    # 💡 修正: with_position 内のタプル直渡しを lambda に変更
    bg_left = ColorClip(size=(420, 720), color=(245, 245, 245)).with_duration(total_duration).with_position(lambda t: (0, 0))
    base_background = CompositeVideoClip([bg_right, bg_left], size=(1280, 720)).with_duration(total_duration)
    clips.append(base_background)

    # --- ② キャラクター層（ロボット背景） ---
    if os.path.exists(ROBOT_BASE_PATH):
        robot_clip = ImageClip(ROBOT_BASE_PATH).resized(new_size=(1280, 720)).with_duration(total_duration).with_position(lambda t: (0, 0))
        clips.append(robot_clip)
    else:
        print(f"⚠️ 警告: ロボットのベース画像が見つかりません: {ROBOT_BASE_PATH}")

    # 瞬きアニメーションのレイヤー
    t = 4.0
    while t < total_duration:
        if os.path.exists(ROBOT_BLINK_PATH):
            blink = ImageClip(ROBOT_BLINK_PATH).resized(new_size=(1280, 720)).with_start(t).with_duration(0.15).with_position(lambda t: (0, 0))
            clips.append(blink)
        t += 4.0

    # --- ③ 常時表示テキスト層 ---
    title_clip = TextClip(text="AI NEWS FLASH", font_size=32, color="black", font=FONT_PATH).with_position(lambda t: (40, 40)).with_duration(total_duration)
    model_clip = TextClip(text=f"Powered by {data['ai_model']}", font_size=16, color="gray", font=FONT_PATH).with_position(lambda t: (950, 40)).with_duration(total_duration)
    ...
    if main_ui_duration > 0:
        ...
        # ニュース見出し用の黄色いボード
        board_clip = ColorClip(size=(360, 430), color=(247, 251, 196)).with_start(main_ui_start).with_duration(main_ui_duration).with_position(lambda t: (30, BOARD_Y))
        clips.append(board_clip)
        
        # 電球アイコン
        if os.path.exists(LIGHTBULB_PATH):
            icon_clip = ImageClip(LIGHTBULB_PATH).resized(height=30).with_start(main_ui_start).with_duration(main_ui_duration).with_position(lambda t: (50, BOARD_Y + 22))
            clips.append(icon_clip)
            
        # ボード内のタイトル「本日のニュース」
        board_title = TextClip(text="本日のニュース", font_size=22, color="black", font=FONT_PATH).with_start(main_ui_start).with_duration(main_ui_duration).with_position(lambda t: (95, BOARD_Y + 25))
        ...
        for period in timeline:
            ...
            for text_idx, news in enumerate(data["news_list"]):
                ...
                # 💡 lambda引数の初期値指定を利用して、ループ内の現在のy_posの値をスコープに安全に閉じ込める
                txt_clip = TextClip(text=disp_text, font_size=font_size, color=text_color, font=FONT_PATH)\
                    .with_start(p_start).with_duration(p_duration).with_position(lambda t, y=y_pos: (45, y))
        ...
        for period in timeline:
            ...
            if os.path.exists(img_path):
                wipe_clip = ImageClip(img_path).resized(width=460).with_start(p_start).with_duration(p_duration).with_position(lambda t: (450, 170))
                clips.append(wipe_clip)
            else:
                print(f"⚠️ 警告: ニュース画像が見つかりません: {img_path}")

    # --- ⑥ 最前面（中の人の愚痴フェーズ） ---
    guchi_bg = ColorClip(size=(1280, 120), color=(0, 0, 0)).with_start(guchi_start).with_duration(total_duration - guchi_start).with_position(lambda t: (0, 600))
    guchi_txt = TextClip(text="【中の人の本音（愚痴フェーズ発動中）】", font_size=20, color="red", font=FONT_PATH).with_start(guchi_start).with_duration(total_duration - guchi_start).with_position(lambda t: (50, 610))
